...
### 5. 非推奨のunique_togetherからUniqueConstraintへ移行(9/8)

従来の仕様ではフォローやLikeなどのユニーク制約にunique_togetherを採用していましたが、Django公式では非推奨であり今後廃止予定であることを知りUniqueConstraintへ移行を
行いました。

### 6. goalshareの振り返りにおいて投稿する完了済みのタスクを選択式に変更(9/17)

従来では振り返りでは完了済みのタスクをすべて投稿する仕様でした。しかし勉強のタスクだけでなく日常的なタスクなどのプライベートな内容もonedayにおいて一括管理することを想定しているため、投稿内容を選択式にする必要性を感じ実装しました。

### 7. Monthlyにtoggle機能を追加

従来の仕様ではtoggle機能が存在せず、その目標が達成したかどうか分かりづらい仕様になっていたため、toggle機能を追加しました。
## AI活用履歴
