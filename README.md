...
「FollowGrow」という名前には、自分と同じような目標を持つ人が成長していく様子を追いかける、という意味を込めています。また、命名の際には中央大学・飯田朝子先生の授業で学んだ「名前をつけるときはスペルや音に共通項があるとよい」という考え方を意識しました。
...
FollowGrowは、「目標達成を継続できるToDoアプリ」です。ToDoリストを作るだけでなく、それを他のユーザーと共有し、他の人が一日にどんなToDoをこなしたかを見られるようにすることで、一人では続けにくい目標達成へのモチベーションを維持できるようにしています。

既存のToDoアプリや勉強記録アプリと異なる点は、主に以下の2つです。

1. **ToDoリストそのものを共有できること**：自分と似た目標を持つ人が、一日に何を頑張ったかを見ることができ、頑張りをお互いに共有できます。
2. **「一日に行ったToDoリスト」に特化していること**：既存の勉強記録アプリは、勉強時間や科目の記録が中心であり、タイムラインには大量の学習ログが流れてきます。FollowGrowでは、あくまで「その日行ったToDo」だけを共有するため、あるユーザーが取り組んでいることの全体像を一目で把握できます。
...
Webアプリ開発、およびDjangoの利用は今回が初めてでした。そのため、まずOneDay Todoの制作を通じて開発の一連の流れ(環境構築からモデル設計、View、Template、デプロイまで)を理解した上で、Monthly ToDoやGoalShareを含めた全体の開発を進める方針としました。
...
- 情報量が多く、学習時のトラブルシューティングがしやすい
- 文法がシンプルで、ロジックや設計そのものに集中できる

### Django

- 認証機能、管理画面、ORMが標準で組み込まれており、開発初期の実装コストを抑えられる
- 今回実装したSNS的機能(タイムライン、フォロー機能)との相性がよい
- Flaskと比較して、短期間での実装に適している

### DB(SQLite → 将来的にPostgreSQL移行を想定)

MVP開発時点ではSQLiteを採用しました。追加のセットアップが不要で、開発速度を優先できるためです。将来的な本番運用を見据え、PostgreSQLへの移行を想定した設計としています。
...
Django標準の`ManyToManyField`だけでも多対多関係は表現できますが、「いつフォローしたか」という付随情報を持たせたかったため、あえて独立したモデルとして設計しました。
...
### 2. 認証機能の追加とデータのユーザー分離

当初はユーザーという概念がなく、全リストが誰からでも操作できる状態でした。認証機能を実装し、各モデルにユーザーとの紐付けを行うことで、自分のデータのみを操作できる設計に変更しました。

### 3. Monthly Todoの仕様変更

当初はOneDay Todoと同様にタスク管理機能を持たせる予定でしたが、「締め切りを管理し、残り日数を可視化する」という役割に特化させ、シンプルな目標管理ツールとして再設計しました。

### 4. GoalShareにおけるタスクのスナップショット化

振り返り投稿時に「その日完了したタスク」をテキストとして保存する設計とし、OneDay_todo側のデータが後から変更・削除されても、過去の投稿内容が影響を受けないようにしました。

### 5. コミュニティ機能からタグ機能への設計転換

**当初の構想**：Community(グループ)モデルを作成し、ユーザーがグループに参加することで同じ目標を持つ人同士がつながれる機能を想定していました。

**方針転換の理由**：グループの作成・参加・退出・権限管理などは実装コストが高く、開発スケジュールを圧迫するリスクがありました。改めて要件を検討した結果、「グループに所属すること」自体が目的ではなく、「似た目標・属性を持つ人の投稿が自然に見つかること」こそがユーザー価値だと判断しました。

**採用した設計**：Profileモデルに`ManyToManyField`でTagを持たせ、ユーザーが複数のタグ(例：「学生」「IT」)を自由に組み合わせて設定できるようにしました。これにより、単一タグでの絞り込みだけでなく、複数タグの組み合わせによる絞り込み、およびタグの包含関係(「学生×IT」のユーザーが「ITのみ」で検索した際にも表示される)を、グループという概念を作らずに実現しました。結果として実装コストを抑えつつ、「同じ志を持つ人とつながる」という当初のコンセプトを満たす設計にたどり着きました。

## AI活用履歴

本プロジェクトの開発にあたり、Claude(Anthropic社のAIアシスタント)をメンター役として活用しました。
...
**主な活用場面**：
- Djangoのモデル設計における考え方の相談(FK設計、related_nameの命名、多対多関係の設計判断など)
- 発生したエラーの原因切り分けと解決
...
- **DM機能**：ユーザー間で直接メッセージをやり取りできる機能。実装コストが高いため、今回のMVPでは見送りました。
- **コメント機能**：GoalShareの投稿に対してコメントできる機能。
